...
function spiltArray<T>(array:Array<T>, numParts: number) {
    ...
    return (
        ...
        >
                    {reviews.concat(reviews).map((imgSrc , reviewIndex) => (
                            <Review key={reviewIndex} className={
...
function ReviewGrid() {
    ...
    const columns = spiltArray(PHONES , 3)
    const column1 = columns[0]
    const column2= columns[1]
    const column3 = spiltArray(columns[2] , 2)
